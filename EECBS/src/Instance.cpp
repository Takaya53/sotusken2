...
// === ここから追加 ===
std::list<int> Instance::getNeighbors3D(int curr3) const
{
    std::list<int> neighbors;
    int row, col, z;
    decode3D(curr3, row, col, z);

    auto push_if_free_sameZ = [&](int nr, int nc) {
        if (nr < 0 || nr >= num_of_rows || nc < 0 || nc >= num_of_cols) return;
        int base = linearizeCoordinate(nr, nc);
        if (!my_map[base]) {
            neighbors.emplace_back(encode3D(nr, nc, z));
        }
    };

    // 回転：z ± 1 へ（回転フェーズの時間消費をグラフ上の1遷移で表現）
    if (z > 0)               neighbors.emplace_back(encode3D(row, col, z - 1));
    if (z + 1 < WEIGHT)      neighbors.emplace_back(encode3D(row, col, z + 1));

    // 前進：
    //  - 縦（N/S）は z == 0 のときのみ許可
    //  - 横（E/W）は z == WEIGHT-1 のときのみ許可
    if (z == 0) {
        // up/down（row-1 / row+1）
        push_if_free_sameZ(row - 1, col);
        push_if_free_sameZ(row + 1, col);
    }
    if (z == WEIGHT - 1) {
        // left/right（col-1 / col+1）
        push_if_free_sameZ(row, col - 1);
        push_if_free_sameZ(row, col + 1);
    }

    // 待機（必要なら）：同一セル・同一zのまま
    // ※ SingleAgentSolver 側でcurrを足している実装なら不要。二重追加を避けたい場合はコメントアウト。
    // neighbors.emplace_back(curr3);
