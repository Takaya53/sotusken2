...
#if DISABLE_MDD
    // MDD/WDG/CG/DG を使わない。ヒューリスティックは 0 に固定（既存 h_val より小さくしない）
    curr.h_val = std::max(curr.h_val, 0);
    // f <= f^ の整合性だけ満たしておく（既存ロジックと同様の安全策）
    curr.cost_to_go = std::max(curr.cost_to_go, 0);
    return true;
#endif
...
bool CBSHeuristic::computeInformedHeuristics(ECBSNode& curr, const vector<int>& min_f_vals, double _time_limit)
{
    curr.h_computed = true;
#if DISABLE_MDD
    // MDD/WDG を使わない（ECBS 版）。ヒューリスティック 0 固定。
    curr.h_val = std::max(curr.h_val, 0);
    curr.cost_to_go = std::max(curr.cost_to_go, 0); // ensure f <= f^ 相当
    return true;
#endif
	// create conflict graph
	start_time = clock();
